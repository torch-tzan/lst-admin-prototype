...
const PracticeLessons = () => {
  ...
  const allPractice = getLessons().filter((l) => l.type === "practice");
  const upcomingLessons = allPractice
    .filter((l) => l.status === "pending" || l.status === "confirmed" || l.status === "in_progress")
    .sort((a, b) => a.date.localeCompare(b.date) || a.startTime.localeCompare(b.startTime));
  const completedLessons = allPractice
      ...
      <div className="flex flex-col h-full bg-background">
        <div className="flex-shrink-0 sticky top-0 z-30 bg-background">
          <div className="px-[20px] pt-4 pb-2 flex items-center justify-center">
            <h1 className="text-xl font-bold text-foreground">実践練習</h1>
          </div>
        </div>
          ...
          {lessons.length === 0 ? (
            <div className="flex flex-col items-center justify-center pt-20 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {activeTab === "upcoming" ? "予定の実践練習はありません" : "完了した実践練習はありません"}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {lesson.venueName && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/venue-detail?name=${encodeURIComponent(lesson.venueName!)}`);
                          }}
                          className="flex items-start gap-0.5 mt-0.5 hover:opacity-70 transition-opacity"
                        >
                          <MapPin className="w-3 h-3 text-accent mt-0.5 flex-shrink-0" />
                          <p className="text-xs text-accent text-left">{lesson.venueName} · {lesson.venueAddress}</p>
                        </button>
                      )}
                    </div>
